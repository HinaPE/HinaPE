#ifndef HINAPE_DETAIL_VECTOR4_INL_H_
#define HINAPE_DETAIL_VECTOR4_INL_H_
...
{
    return HinaPE::absmin(HinaPE::absmin(x, y), HinaPE::absmin(z, w));
}

template<typename T>
T Vector<T, 4>::absmax() const
{
    return HinaPE::absmax(HinaPE::absmax(x, y), HinaPE::absmax(z, w));
}

template<typename T>
size_t Vector<T, 4>::dominantAxis() const
{
    return (std::fabs(x) > std::fabs(y)) ? ((std::fabs(x) > std::fabs(z)) ? ((std::fabs(x) > std::fabs(w)) ? 0 : 3) : ((std::fabs(z) > std::fabs(w)) ? 2 : 3)) : ((std::fabs(y) > std::fabs(z)) ? ((std::fabs(y) > std::fabs(w)) ? 1 : 3) : ((std::fabs(z) > std::fabs(w)) ? 2
                                                                                                                                                                                                                                                                           : 3));
}
...
#endif  // HINAPE_DETAIL_VECTOR4_INL_H_
