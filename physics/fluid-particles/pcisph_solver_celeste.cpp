    ...
    Util::parallelFor(Constant::ZeroSize, fluid_size, [&](size_t i)
    {
        ...
        for (size_t j: nl[i])
        {
            ...
            }else
            {
                ...
                {
                    density += bm[j - fluid_size] * poly6((x[i] - b[j - fluid_size]).length());
                }
            }
        }
        ...
        for (size_t j: nl[i])
        {
            ...
            } else
            {
                if (_opt.use_akinci2012_collision)
                {
                    ...
                    if (dist > HinaPE::Constant::Epsilon && bm[j - fluid_size] > HinaPE::Constant::Epsilon)
                    {
                        mVector3 dir = (b[j - fluid_size] - x[i]) / dist;
                        ////////TODO: check this
                        p_f[i] -= m * bm[j - fluid_size] * (p[i] / (d_p[i] * d_p[i])) * spiky.gradient(dist, dir);
                        ///////ADD
                    }
                }
            }
...
auto HinaPE::PCISPHSolverCELESTE::_compute_delta() const -> real
{
        ...
        while (xj[1] <= supportRadius)
        {
            while (xj[2] <= supportRadius)
            {
                ...
                if ((xi - xj).length() * (xi - xj).length() < supportRadius*supportRadius)
                {
                    ...
                    mVector3 dir = (dist > 0) ? (xi - xj) / dist : mVector3::Zero();
    ...
    real denom = 0;
    denom += -sumGradW.dot(sumGradW) - sumGradW2;
