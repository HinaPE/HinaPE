...
#include "kernel/physics_helpers.h"
...
auto SphSolver3::eosExponent() const -> double
{
    return _eosExponent;
}
...
auto SphSolver3::negativePressureScale() const -> double
{
...
auto SphSolver3::viscosityCoefficient() const -> double
{
...
auto SphSolver3::pseudoViscosityCoefficient() const -> double
{
...
auto SphSolver3::speedOfSound() const -> double
{
...
auto SphSolver3::timeStepLimitScale() const -> double
{
...
auto SphSolver3::sphSystemData() const -> SphSystemData3Ptr
{
    return std::dynamic_pointer_cast<SphSystemData3>(particleSystemData());
}

auto SphSolver3::numberOfSubTimeSteps(double timeIntervalInSeconds) const -> unsigned int
{
...
void SphSolver3::onEndAdvanceTimeStep(double timeStepInSeconds)
{
    ...
    JET_INFO << "Max density: " << maxDensity << " " << "Max density / target density ratio: " << maxDensity / particles->targetDensity();
}
...
void SphSolver3::computePressure() const
{
    auto particles = sphSystemData();
    size_t numberOfParticles = particles->numberOfParticles();
    auto d = particles->densities();
...
void SphSolver3::accumulatePressureForce(const ConstArrayAccessor1<Vector3D> &positions, const ConstArrayAccessor1<double> &densities, const ConstArrayAccessor1<double> &pressures, ArrayAccessor1<Vector3D> pressureForces) const
{
    auto particles = sphSystemData();
    size_t numberOfParticles = particles->numberOfParticles();
...
void SphSolver3::accumulateViscosityForce() const
{
    auto particles = sphSystemData();
    size_t numberOfParticles = particles->numberOfParticles();
    auto x = particles->positions();
...
void SphSolver3::computePseudoViscosity(double timeStepInSeconds) const
{
    auto particles = sphSystemData();
    size_t numberOfParticles = particles->numberOfParticles();
    auto x = particles->positions();
...
auto SphSolver3::builder() -> SphSolver3::Builder
{
    return {};
}

auto SphSolver3::Builder::build() const -> SphSolver3
{
    return {_targetDensity, _targetSpacing, _relativeKernelRadius};
}

auto SphSolver3::Builder::makeShared() const -> SphSolver3Ptr
{
    return {new SphSolver3(_targetDensity, _targetSpacing, _relativeKernelRadius), [](SphSolver3 *obj)
    {
        delete obj;
    }};
}
