...
{
    return parallelReduce(kZeroSize, rows() * cols(), T(0), [&](size_t start, size_t end, T init)
    {
        T result = init;
...
T MatrixMxN<T>::min() const
{
    ...
    return parallelReduce(kZeroSize, rows() * cols(), std::numeric_limits<T>::max(), [&](size_t start, size_t end, T init)
    {
        T result = init;
        for (size_t i = start; i < end; ++i)
        {
...
T MatrixMxN<T>::max() const
{
    ...
    return parallelReduce(kZeroSize, rows() * cols(), std::numeric_limits<T>::min(), [&](size_t start, size_t end, T init)
    {
        T result = init;
        for (size_t i = start; i < end; ++i)
        {
