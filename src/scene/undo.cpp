...
Scene_Object &Undo::add_obj(Halfedge_Mesh &&mesh, std::string name, bool is_rigidbody)
{
    Scene_ID id = scene.add({}, std::move(mesh), name, 0, is_rigidbody);
    scene.restore(id);
    action([id, this]()
           { scene.restore(id); },
           [id, this]()
...
Scene_Object &Undo::add_obj(GL::Mesh &&mesh, std::string name, bool is_rigidbody)
{
    Scene_ID id = scene.add({}, std::move(mesh), name, 0, is_rigidbody);
    scene.restore(id);
    action([id, this]()
