...
void Hina::GridFluidSolver::compute_external_forces(float dt)
{
	...
	if (_opt.gravity.x() > Constant::Epsilon)
		_grids->_velocity->for_each_u_index([&](size_t i, size_t j, size_t k) { vel->u(i, j, k) += _opt.gravity.x() * dt; });
	if (_opt.gravity.y() > Constant::Epsilon)
		_grids->_velocity->for_each_v_index([&](size_t i, size_t j, size_t k) { vel->v(i, j, k) += _opt.gravity.y() * dt; });
	if (_opt.gravity.z() > Constant::Epsilon)
		_grids->_velocity->for_each_w_index([&](size_t i, size_t j, size_t k) { vel->w(i, j, k) += _opt.gravity.z() * dt; });
