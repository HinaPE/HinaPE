...
TriangleMesh3::TriangleMesh3(const PointArray &points, const NormalArray &normals, const UvArray &uvs, const IndexArray &pointIndices, const IndexArray &normalIndices, const IndexArray &uvIndices, const Transform3 &transform_, bool isNormalFlipped_) : Surface3(
        transform_, isNormalFlipped_), _points(points), _normals(normals), _uvs(uvs), _pointIndices(pointIndices), _normalIndices(normalIndices), _uvIndices(uvIndices) {}
...
double TriangleMesh3::fastWindingNumber(const Vector3D &q, size_t rootNodeIndex, double accuracy) const
{
    ...
    const Vector3D treeRVec = HinaPE::max(treeP - treeBound.lowerCorner, treeBound.upperCorner - treeP);
