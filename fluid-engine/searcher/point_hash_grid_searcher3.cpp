...
void PointHashGridSearcher3::forEachNearbyPoint(const Vector3D &origin, double radius, const std::function<void(size_t, const Vector3D &
)>& callback) const {
if (_buckets.
empty()
) {
...
const std::vector<std::vector<size_t>> &PointHashGridSearcher3::buckets() const
{
    return _buckets;
}
...
void PointHashGridSearcher3::deserialize(const std::vector<uint8_t> &buffer)
{
    ...
    for (uint32_t i = 0; i < fbsBuckets->size(); ++i)
    {
        ...
        std::transform(fbsBucket->data()->begin(), fbsBucket->data()->end(), _buckets[i].begin(), [](uint64_t val)
        {
...
{
    return std::shared_ptr<PointHashGridSearcher3>(new PointHashGridSearcher3(_resolution, _gridSpacing), [](PointHashGridSearcher3 *obj)
    {
        delete obj;
    });
}
