...
{
    return parallelReduce(kZeroSize, numberOfNonZeros(), T(0), [&](size_t start, size_t end, T init)
    {
        T result = init;
...
T MatrixCsr<T>::min() const
{
    ...
    return parallelReduce(kZeroSize, numberOfNonZeros(), std::numeric_limits<T>::max(), [&](size_t start, size_t end, T init)
    {
        T result = init;
        for (size_t i = start; i < end; ++i)
        {
...
T MatrixCsr<T>::max() const
{
    ...
    return parallelReduce(kZeroSize, numberOfNonZeros(), std::numeric_limits<T>::min(), [&](size_t start, size_t end, T init)
    {
        T result = init;
        for (size_t i = start; i < end; ++i)
        {
...
{
    JET_ASSERT(isSquare());
    return parallelReduce(kZeroSize, rows(), T(0), [&](size_t start, size_t end, T init)
    {
        T result = init;
        for (size_t i = start; i < end; ++i)
        {
