...
public:
    ...
    BBox bbox() const
    {
        BBox box = std::visit([](const auto &o)
    ...
    Trace hit(Ray ray) const
    {
        ...
        Trace ret = std::visit([&ray](const auto &o)
    ...
    Vec3 sample(Vec3 from) const
    {
        ...
        Vec3 dir =
                std::visit(overloaded{[from](const List<Object> &list)
                                      { return list.sample(from); },
                                      [from](const Tri_Mesh &mesh)
                                      { return mesh.sample(from); },
                                      [](const auto &) -> Vec3
                                      {
    ...
    float pdf(Ray ray, Mat4 T = Mat4::I, Mat4 iT = Mat4::I) const
    {
        ...
        return std::visit(
                overloaded{[ray, T, iT](const List<Object> &list)
                           { return list.pdf(ray, T, iT); },
                           [ray, T, iT](const Tri_Mesh &mesh)
                           { return mesh.pdf(ray, T, iT); },
                           [](const auto &) -> float
                           {
                               die("Sampling implicit objects/BVHs is not yet supported.");
                           }},
