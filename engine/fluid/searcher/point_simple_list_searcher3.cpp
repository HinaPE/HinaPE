...
void PointSimpleListSearcher3::forEachNearbyPoint(const Vector3D &origin, double radius, const std::function<void(size_t, const Vector3D &
)>& callback) const {
...
{
    return std::shared_ptr<PointSimpleListSearcher3>(new PointSimpleListSearcher3(), [](PointSimpleListSearcher3 *obj)
    {
        delete obj;
    });
}
