...
#include "kernel/generated/scalar_grid2_generated.h"
...
{
    parallelFor(kZeroSize, _data.width(), kZeroSize, _data.height(), [this, value](size_t i, size_t j) { _data(i, j) = value; }, policy);
}
...
void ScalarGrid2::fill(const std::function<double(const Vector2D &)> &func, ExecutionPolicy policy)
{
    ...
    parallelFor(kZeroSize, _data.width(), kZeroSize, _data.height(), [this, &func, &pos](size_t i, size_t j)
    {
